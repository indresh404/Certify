...
from app.core.renderer import measure_text, calculate_text_position, auto_fit_font_size
...
def get_unique_filename(base_name: str, output_folder: Path, generated_names: set) -> Path:
    """
...
class GeneratorThread(Thread):
    ...
    def run(self):
        if not self.state.template_path or not self.state.output_folder or not self.state.names:
            self.progress_queue.put({"type": "error", "message": "Missing required data for generation."})
            return
        ...
        for i, name in enumerate(self.state.names):
            ...
            try:
                ...
                with Image.open(self.state.template_path) as img:
                    ...
                    font = ImageFont.truetype(font_path, final_size) if font_path else ImageFont.load_default()
                    ...
                    # 4. Draw text
                    draw.text((draw_x, draw_y), name, font=font, fill=self.state.text_color)
                    
                    # 5. Determine unique filename
                    # Note: We fallback to row number if sanitize leaves it empty
                    base_name = name if sanitize_filename(name) else f"Certificate_{i+1}"
                    output_path = get_unique_filename(base_name, self.state.output_folder, generated_files)
