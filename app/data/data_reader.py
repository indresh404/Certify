...
def _load_excel(excel_path: Path) -> Tuple[List[str], List[Dict[str, str]]]:
    try:
        ...
        for i, row in enumerate(sheet.iter_rows(values_only=True)):
            if i == 0:
                headers = [str(cell).strip() if cell is not None else f"Column_{j+1}" for j, cell in enumerate(row)]
                if not headers:
                    raise DataError("No headers found in the Excel file.")
            else:
                row_dict = {}
                for j, h in enumerate(headers):
                    val = str(row[j]).strip() if j < len(row) and row[j] is not None else ""
                    row_dict[h] = val
                rows.append(row_dict)
        ...
        raise DataError(f"Unable to read the Excel file: {str(e)}")

def extract_names(rows: List[Dict[str, str]], column_name: str) -> Tuple[List[str], int]:
    names = []
    seen = set()
    duplicates = 0
