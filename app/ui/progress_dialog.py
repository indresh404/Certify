...
class CompletionDialog(ctk.CTkToplevel):
    def __init__(
        self, master, completed, failed, total, errors, output_folder, **kwargs
    ):
        super().__init__(master, **kwargs)
        ...
        ctk.CTkButton(
            btn_frame, text="Open Output Folder", command=self.open_folder
        ).pack(side="left", padx=5)
        ctk.CTkButton(btn_frame, text="Close", command=self.destroy).pack(
            side="left", padx=5
        )
