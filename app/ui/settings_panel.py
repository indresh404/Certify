...
class SettingsPanel(ctk.CTkFrame):
    def __init__(self, master, state: AppState, on_change_callback, on_action_callback, **kwargs):
        ...
        # Template Section
        self.lbl_template = ctk.CTkLabel(self, text="TEMPLATE", font=ctk.CTkFont(weight="bold"))
        self.lbl_template.grid(row=0, column=0, sticky="w", padx=10, pady=(10, 0))
        
        self.btn_upload_tpl = ctk.CTkButton(self, text="Upload Template", command=self.upload_template)
        ...
        # Data Section
        self.lbl_csv = ctk.CTkLabel(self, text="DATA (CSV / Excel)", font=ctk.CTkFont(weight="bold"))
        self.lbl_csv.grid(row=3, column=0, sticky="w", padx=10, pady=(20, 0))
        
        self.btn_upload_csv = ctk.CTkButton(self, text="Upload Data File", command=self.upload_csv)
        ...
        self.col_dropdown = ctk.CTkOptionMenu(col_frame, variable=self.col_var, values=[""], command=self.column_changed)
        ...
        # Settings Section
        self.lbl_settings = ctk.CTkLabel(self, text="NAME SETTINGS", font=ctk.CTkFont(weight="bold"))
        ...
        self.font_dropdown = ctk.CTkOptionMenu(font_frame, variable=self.font_var, command=self.font_changed)
        self.font_dropdown.grid(row=0, column=1, sticky="ew", padx=(10, 0))
        
        self.btn_browse_font = ctk.CTkButton(font_frame, text="Browse...", width=60, command=self.browse_font)
        ...
        ctk.CTkButton(size_frame, text="-", width=30, command=lambda: self.adjust_size(-1)).grid(row=0, column=2, padx=2)
        ctk.CTkButton(size_frame, text="+", width=30, command=lambda: self.adjust_size(1)).grid(row=0, column=3, padx=2)
        ...
        self.btn_color = ctk.CTkButton(color_align_frame, text="■ Select", width=80, command=self.pick_color)
        ...
        self.align_dropdown = ctk.CTkOptionMenu(color_align_frame, variable=self.align_var, values=["left", "center", "right"], width=80, command=self.align_changed)
        self.align_dropdown.grid(row=0, column=3, padx=(5, 0))
        
        # Auto-fit
        self.autofit_var = ctk.BooleanVar(value=self.state.auto_fit)
        self.chk_autofit = ctk.CTkCheckBox(self, text="Auto-fit to width", variable=self.autofit_var, command=self.autofit_changed)
        ...
        ctk.CTkButton(reset_frame, text="Reset Pos", command=lambda: self.on_action("reset_pos")).grid(row=0, column=0, padx=(0, 5), sticky="ew")
        ctk.CTkButton(reset_frame, text="Reset All", command=lambda: self.on_action("reset_all")).grid(row=0, column=1, padx=(5, 0), sticky="ew")
        
        # Output
        self.lbl_output = ctk.CTkLabel(self, text="OUTPUT", font=ctk.CTkFont(weight="bold"))
        self.lbl_output.grid(row=13, column=0, sticky="w", padx=10, pady=(20, 0))
        
        self.btn_output = ctk.CTkButton(self, text="Select Folder", command=self.select_output)
        ...
        # Generate
        self.btn_generate = ctk.CTkButton(self, text="GENERATE", height=40, font=ctk.CTkFont(weight="bold"), command=lambda: self.on_action("generate"))
