...
class MainWindow(ctk.CTk):
    def __init__(self):
        ...
        ctk.CTkButton(
            ...
            text="Fit",
            width=50,
            command=lambda: self.canvas.set_zoom("fit"),
        ).pack(side="right", padx=2)
        ctk.CTkButton(
            zoom_frame,
            text="-",
            width=30,
            command=lambda: self.canvas.set_zoom("step", -0.1),
        ).pack(side="right", padx=2)
        ctk.CTkButton(
            zoom_frame,
            text="+",
        ...
        self.gen_lbl_status = ctk.CTkLabel(
            self.gen_left,
        ...
        self.gen_progress = ctk.CTkProgressBar(
            self.gen_left, mode="determinate", height=15
        )
        self.gen_progress.pack(fill="x", padx=30, pady=10)
        self.gen_progress.set(0)

        self.gen_lbl_stats = ctk.CTkLabel(
            self.gen_left,
            text="Completed: 0 / 0\nFailed: 0\nRemaining: 0",
            justify="center",
        ...
        self.gen_btn_cancel = ctk.CTkButton(
            self.gen_left,
            text="Cancel",
        ...
        ctk.CTkLabel(
            self.gen_right, text="Generation Logs", font=ctk.CTkFont(weight="bold")
        ).pack(pady=5)
    ...
    def setup_about_tab(self):
        ...
        if profile_path.exists():
            ...
            self.profile_img = ctk.CTkImage(
                light_image=img, dark_image=img, size=(150, 150)
            )
        ...
        ctk.CTkLabel(
            self.about_tab, text="GitHub: @indresh404 (https://github.com/indresh404)"
        ).pack(pady=5)
        ctk.CTkLabel(
            self.about_tab, text="Project: Certify : Certificate Generator"
        ).pack(pady=5)
    ...
    def animate_gif(self, path):
        ...
        def update_frame(idx):
            ...
            self.after(
                self.gif_durations[idx], update_frame, (idx + 1) % len(self.gif_frames)
            )
    ...
    def start_generation(self):
        ...
        if not os.access(self.app_state.output_folder, os.W_OK):
            messagebox.showerror(
                "Error",
                "The selected output folder is not writable. Please choose another folder.",
            )
            return
        ...
        self.generator_thread = GeneratorThread(
            self.app_state, self.generator_queue, self.cancel_event
        )
        ...
        try:
            while True:
                ...
                if msg["type"] == "progress":
                    ...
                    self.gen_log_textbox.configure(state="normal")
                    self.gen_log_textbox.insert(
                        "end", f"Generated: {msg['current_name']}\n"
                    )
                    ...
                    elapsed = time.time() - self.generation_start_time
                    elapsed_str = time.strftime("%M:%S", time.gmtime(elapsed))
                    self.gen_lbl_stats.configure(
                        text=f"Completed: {c} / {t}\nFailed: {f}\nRemaining: 0\n\nElapsed: {elapsed_str}\nETA: 00:00"
                    )
                    self.gen_log_textbox.configure(state="normal")
                    self.gen_log_textbox.insert(
                    ...
                    CompletionDialog(
                        self, c, f, t, msg["errors"], self.app_state.output_folder
                    )
                    return
