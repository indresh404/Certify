...
from app.core.renderer import measure_text, calculate_text_position
...
class PreviewCanvas(ctk.CTkCanvas):
    ...
    def set_zoom(self, mode: str, step: float = 0.0):
        if mode == "fit":
            ...
            self.state.preview_scale = max(0.1, min(10.0, self.state.preview_scale + step))
    ...
    def update_preview(self, fit_first=False):
        if not self.state.template_path:
            ...
            self.create_text(
                ...
                text="Upload a template to begin", fill="gray"
            )
            return
    ...
    def draw_text_overlay(self):
        ...
        if self.bbox_id: self.delete(self.bbox_id)
        if self.h_guide: self.delete(self.h_guide)
        if self.v_guide: self.delete(self.v_guide)
        ...
        if self.state.auto_fit and font_path:
            try:
                ...
                final_size = auto_fit_font_size(draw, preview_name, font_path, self.state.font_size, max_width)
            except Exception:
                pass

        # Convert original coordinates to preview coordinates
        px, py = original_to_preview(self.state.text_x, self.state.text_y, scale, offset)
        ...
        try:
            font = ImageFont.truetype(font_path, int(final_size * scale)) if font_path else ImageFont.load_default()
            ...
            text_draw.text((-bbox[0]+5, -bbox[1]+5), preview_name, font=font, fill=self.state.text_color)
            
            self.tk_text_img = ImageTk.PhotoImage(text_img)
            self.text_id = self.create_image(draw_px-5, draw_py-5, anchor="nw", image=self.tk_text_img)
            
            # Draw bounding box
            self.bbox_id = self.create_rectangle(
                draw_px, draw_py, draw_px + w, draw_py + h,
            ...
            if self.is_dragging:
                ...
                if abs(px - canvas_center_x) < 10:
                    self.v_guide = self.create_line(canvas_center_x, 0, canvas_center_x, self.winfo_height(), fill="red", dash=(2, 2))
                if abs(py - canvas_center_y) < 10:
                    self.h_guide = self.create_line(0, canvas_center_y, self.winfo_width(), canvas_center_y, fill="red", dash=(2, 2))
                    
        except Exception as e:
            print(f"Error drawing text preview: {e}")
            
    def on_press(self, event):
        if not self.state.template_path: return
    ...
    def on_drag(self, event):
        ...
        # Clamp to canvas
        px, py = original_to_preview(self.state.text_x, self.state.text_y, scale, self.state.preview_offset)
        ...
        self.state.text_x, self.state.text_y = preview_to_original(px, py, scale, self.state.preview_offset)
